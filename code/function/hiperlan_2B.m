...
% delay in ns
dTap = [0 10 20 30 50 80 110 140 180 230 280 330 380 430 490 560 640 730] * 1e-9;

% average relative power
pTapDb = [-2.6 -3.0 -3.5 -3.9 0.0 -1.3 -2.6 -3.9 -3.4 -5.6 -7.7 -9.9 -12.1 -14.3 -15.4 -18.4 -20.7 -24.6];
pTap = db2pow(pTapDb);
...
fading = sqrt(1 / 2) * (randn(size(pTap)) + 1i * randn(size(pTap)));
gTap = sqrt(pTap) .* fading;
